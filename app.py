...
from config import FLITT_CONFIG, TBC_CONFIG, TBC_ECOMMERCE_CONFIG, IS_PRODUCTION
...
def create_tbc_ecommerce_payment(product_id, amount, user_ip_address=None):
    """Create TBC E-Commerce payment using method 5 (installment)"""
    try:
        ...
        payment_data = {
            ...
            "userIpAddress": user_ip_address or request.remote_addr,
...
def tbc_ecommerce_payment():
    """Handle TBC E-Commerce payment creation"""
    try:
        print("=== TBC E-COMMERCE PAYMENT REQUEST ===")
        print(f"Request Method: {request.method}")
        print(f"Request URL: {request.url}")
...
def tbc_ecommerce_callback():
    """Handle callback from TBC E-Commerce payment"""
    try:
        ...
        print(f"TBC E-Commerce callback: Payment ID {payment_id}")
        
        # Here you would typically verify the payment status with TBC
        # For now, we'll redirect to payment result page
        return redirect(url_for('payment_result', status='success', payment_type='tbc_ecommerce'))
            
    except Exception as e:
        print(f"TBC E-Commerce callback error: {str(e)}")
        return redirect(url_for('payment_result', status='failure', payment_type='tbc_ecommerce'))
